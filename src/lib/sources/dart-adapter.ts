...
export class OpenDartAdapter implements SourceAdapter {
  ...
  constructor(private apiKey = process.env.OPENDART_API_KEY, private fetcher: FetchLike = fetch) {}

  async fetchDocuments(input: FetchDocumentsInput = {}): Promise<FetchDocumentsResult> {
    if (!this.apiKey) {
      return { source: this.source, availability: { ok: false, reason: "OPENDART_API_KEY is not configured" }, documents: [], warnings: ["DART live adapter is disabled until OPENDART_API_KEY is set server-side."] };
    }
    ...
    if (input.startDate) url.searchParams.set("bgn_de", input.startDate.replaceAll("-", ""));
    if (input.endDate) url.searchParams.set("end_de", input.endDate.replaceAll("-", ""));
